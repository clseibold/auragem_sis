...
func GetTopics(conn *sql.DB) []Topic {
	query := "SELECT topics.ID, topics.title, topics.description, topics.date_added, COUNT(questions.id) FROM topics LEFT JOIN questions ON questions.topicid=topics.id GROUP BY topics.ID, topics.title, topics.description, topics.date_added, topics.\"order\" ORDER BY topics.\"order\" ASC"
